...
const EXCLUSIVE_THEMES = {
  ...
  neonMatrix: {
    ...
    accent: "bg-emerald-600 hover:bg-emerald-500 shadow-emerald-500/20 text-white",
...
function Header({ profile }) {
  ...
  const handlePublishWorkspace = (e) => {
    e.preventDefault();
    
    // Evaluate if they left custom sites blank to handle the default fallback logic
    const parsingSitesMode = integrationData.customSites.trim() 
      ? `Custom projects target list synced: "${integrationData.customSites}"` 
      : "No custom sites provided. Falling back to your original hardcoded baseline templates by default.";

    alert(
      `Ecosystem Instance Provisioned & Uploaded Successfully!\n\n` +
      ` Node Host: https://${subdomain}.devhub.ng\n` +
      ` WhatsApp Linked: ${integrationData.whatsappHandle || "None Provided"}\n` +
      ` Telegram Linked: ${integrationData.telegramHandle || "None Provided"}\n` +
      ` Status: ${parsingSitesMode}`
    );
    ...
    setSubdomain("");
    setIntegrationData({ whatsappHandle: "", telegramHandle: "", customSites: "" });
  };
    ...
    <>
      ...
      <header className="sticky top-0 z-50 w-full border-b border-purple-100 bg-white/80 backdrop-blur-md transition-colors duration-300 dark:border-gray-800 dark:bg-gray-900/80">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          ...
          <div className="flex items-center">
            ...
            >
              <Sun size={16} className={`relative transition-all duration-500 ${theme === "dark" ? "-rotate-90 scale-0 opacity-0 absolute" : "rotate-0 scale-100 opacity-100"}`} />
              <Moon size={16} className={`relative transition-all duration-500 ${theme === "light" ? "rotate-90 scale-0 opacity-0 absolute" : "rotate-0 scale-100 opacity-100"}`} />
              <span className="hidden sm:block relative overflow-hidden">
                <span className={`block text-xs font-medium transition-all duration-500 ${theme === "dark" ? "-translate-y-full opacity-0" : "translate-y-0 opacity-100"}`}>Light</span>
                <span className={`absolute inset-0 text-xs font-medium transition-all duration-500 ${theme === "light" ? "translate-y-full opacity-0" : "translate-y-0 opacity-100"}`}>Dark</span>
              </span>
            </button>
          </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4 transition-all">
          <div className={`relative w-full max-w-xl overflow-hidden rounded-2xl border bg-gradient-to-br ${option === "copy" ? `${currentActiveTheme.bg} ${currentActiveTheme.border}` : "bg-white border-gray-100 dark:bg-gray-900 dark:border-gray-800"} p-5 sm:p-6 shadow-2xl transition-all duration-300 animate-in fade-in zoom-in-95`}>
            ...
            {!option && (
              <div>
                ...
                <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-6">
                  Choose how you want to interact with our infrastructure ecosystem.
                </p>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[11px] sm:text-xs text-gray-600 dark:text-gray-400 mt-1">
                        Submit your basic contact info and route directly to WhatsApp or Telegram to kickstart your project plan.
                      </p>
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[11px] sm:text-xs text-gray-500 dark:text-gray-400 mt-1">
                        Duplicate this codebase via GitHub or type manual node configurations to deploy directly into your sandbox profile dynamically.
                      </p>
                    </div>
                  </button>
                </div>
            ...
            {option === "build" && (
              <form onSubmit={handleBuildSubmit} className="space-y-4">
                <div>
                  <h3 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white">Project Brief Details</h3>
                  <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Provide contact info to link directly to your messenger choice.</p>
                </div>
                <div className="space-y-3">
                  <div>
                    <label className="block text-[11px] sm:text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Full Name</label>
                    <input required type="text" name="name" value={formData.name} onChange={handleInputChange} placeholder="John Doe" className="w-full rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-white px-3 sm:px-4 py-2 sm:py-2.5 text-sm focus:border-purple-500 focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-[11px] sm:text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Email Address</label>
                    <input required type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="john@example.com" className="w-full rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-white px-3 sm:px-4 py-2 sm:py-2.5 text-sm focus:border-purple-500 focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-[11px] sm:text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Phone Number</label>
                    <input required type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="+234..." className="w-full rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-white px-3 sm:px-4 py-2 sm:py-2.5 text-sm focus:border-purple-500 focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-[11px] sm:text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Preferred Platform</label>
                    <div className="grid grid-cols-2 gap-2 mt-1">
                      <label className={`flex items-center justify-center p-2.5 sm:p-3 rounded-xl border cursor-pointer font-medium text-xs sm:text-sm transition ${formData.platform === "whatsapp" ? "border-purple-600 bg-purple-50 text-purple-600 dark:bg-purple-950/40 dark:text-purple-400" : "border-gray-200 text-gray-600 dark:border-gray-700 dark:text-gray-400 hover:bg-gray-50"}`}>
                        <input type="radio" name="platform" value="whatsapp" checked={formData.platform === "whatsapp"} onChange={handleInputChange} className="sr-only" />
                        WhatsApp
                      </label>
                      <label className={`flex items-center justify-center p-2.5 sm:p-3 rounded-xl border cursor-pointer font-medium text-xs sm:text-sm transition ${formData.platform === "telegram" ? "border-purple-600 bg-purple-50 text-purple-600 dark:bg-purple-950/40 dark:text-purple-400" : "border-gray-200 text-gray-600 dark:border-gray-700 dark:text-gray-400 hover:bg-gray-50"}`}>
                        <input type="radio" name="platform" value="telegram" checked={formData.platform === "telegram"} onChange={handleInputChange} className="sr-only" />
                        Telegram
                      </label>
                    </div>
                  </div>
                ...
                <div className="flex gap-2 pt-2">
                  <button type="button" onClick={() => setOption(null)} className="w-1/3 rounded-xl border border-gray-200 dark:border-gray-700 dark:text-gray-300 text-xs sm:text-sm font-semibold text-gray-600 py-2 sm:py-2.5">Back</button>
                  <button type="submit" className="w-2/3 rounded-xl bg-purple-600 text-xs sm:text-sm font-semibold text-white hover:bg-purple-700 py-2 sm:py-2.5">Connect Messenger</button>
                </div>
              </form>
            )}
            ...
            {option === "copy" && (
              <div className="text-white space-y-5">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <Settings className={`animate-spin [animation-duration:8s] ${currentActiveTheme.text}`} size={18} />
                    <span className={`text-xs font-bold uppercase tracking-widest ${currentActiveTheme.text}`}>Automated Schema Core</span>
                  </div>
                  <h3 className="text-xl font-extrabold tracking-tight mt-1">Fork Full Infrastructure</h3>
                  <p className="text-xs text-slate-400 mt-0.5">Deploy this responsive app layout wrapper directly into an editable structure.</p>
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {Object.entries(EXCLUSIVE_THEMES).map(([key, item]) => (
                      ...
                      >
                        <span>{item.name}</span>
                        <span className={`h-2.5 w-2.5 rounded-full ${item.accent.split(" ")[0]}`} />
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="min-h-[180px]">
                  ...
                  {copyStep === "selection" && (
                    <div className="space-y-3">
                      ...
                      <div className={`border rounded-xl p-3 text-xs font-medium flex items-center gap-2 ${currentActiveTheme.pill}`}>
                        <Sparkles size={14} className="shrink-0" />
                        Ready to launch. Choose your replication pipeline method below to spin up the asset wrapper.
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-2">
                        <button
                          type="button"
                          onClick={() => {
                            window.open("https://github.com/OPEmma/command-center-template", "_blank");
                            setIsOpen(false);
                            setOption(null);
                          }}
                  ...
                  {copyStep === "manualForm" && (
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Workspace Hub Name</label>
                          <input 
                            type="text" 
                            name="siteName"
                        ...
                        <div>
                          <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Developer Handle Title</label>
                          <input 
                            type="text" 
                            name="developerTitle"
                      ...
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Source Repository Target URL (Optional)</label>
                        <input 
                          type="url" 
                          name="repoUrl"
                      ...
                      <div className="flex gap-3 pt-4">
                        <button type="button" onClick={() => setCopyStep("selection")} className="w-1/3 rounded-xl border border-slate-800 text-xs font-bold text-slate-400 hover:bg-white/5 py-2.5">Back</button>
                        <button 
                          type="button" 
                          onClick={() => setCopyStep("domainPopup")}
                          disabled={!customWorkspace.siteName || !customWorkspace.developerTitle}
                  ...
                  {copyStep === "domainPopup" && (
                    <div className="space-y-4 text-center py-2">
                      ...
                      <div>
                        <h4 className="text-base font-bold tracking-tight">Claim Your Network Identity</h4>
                        <p className="text-xs text-slate-400 max-w-xs mx-auto mt-0.5">Assign a secure subdomain prefix to route your compiled sandbox environment files.</p>
                      </div>
                      ...
                      <div className="flex justify-center gap-3 pt-3">
                        <button type="button" onClick={() => setCopyStep("manualForm")} className="px-5 py-2 text-xs font-bold border border-slate-800 rounded-xl text-slate-400 hover:bg-white/5">Back</button>
                        <button 
                          type="button" 
                          disabled={!subdomain}
                  ...
                  {copyStep === "integrations" && (
                    <form onSubmit={handlePublishWorkspace} className="space-y-3 text-left animate-in fade-in zoom-in-95 duration-200">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">WhatsApp Handle / No.</label>
                          <input 
                            type="text"
                            name="whatsappHandle"
                        ...
                        <div>
                          <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Telegram Username</label>
                          <input 
                            type="text"
                            name="telegramHandle"
